...
# These fail sometimes bc kernel can't re-read part table
fdisk /dev/sda <<EOF
o
n
p
...
set +x
echo -n $ENCPASS | cryptsetup luksFormat --type luks1 /dev/sda1 --key-file -
...
dnf install -y --releasever=31 --installroot=/mnt/sysimage filesystem
...
dnf install -y --installroot=/mnt/sysimage --disablerepo=* --enablerepo=dvd @core @standard kernel btrfs-progs lvm2 grub2-pc-modules grub2-tools 
...
# put in place symlink for /etc/grub2.cfg. If we don't
# then grub doesn't get updated when new kernels get installed
pushd /etc/
